...
library(pROC)
library(ggplot2)
#glm_ROC=roc(ctest[ ,1], diab.glm_prob, legacy.axces=TRUE)
#ggroc(glm_ROC) + ggtitle("ROC curve")
...
egentlig står det i module pagene til M4 at man buker KNN når class boundaries er non-linear, kanskje de erdet her? er litt usikker på det egentlig. 
...
We choos to use random forest to analyse the data. 

Write out themodel: det er jo litt vanskelig når det er en forest?? 
Model assumptions: 
Tuning parameters: m=ant predictorer som averages over i hver split. m=sqrt(p), ca. Og antall trær, må være stor nok til at erroren har flatet ut.
velger derfor m=2 eller 3
Insight into the interpretation of the fitted model:
Evaluation of the model:
```{r}
library(randomForest)
set.seed(1)
#diab.rf = randomForest(diabetes ~ ., data=ctrain, mtry=3, ntree=100, importance=TRUE) #dårig ide, dette funker jo ikke. 
#diab.rf$confusion
diab.bagging=randomForest(diabetes ~ ., data = ctrain, mtry=7, ntree=500, importance=TRUE)
diab.bagging$confusion
