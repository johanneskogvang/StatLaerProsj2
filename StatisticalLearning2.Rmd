...
mis_class_range = linspace(0,0.4,101)
...
```
We see that the logistic regression has the lowest misclassification rate, which is a good indication that this is the best method for our problem. We can also compare the AUCs for the methods where this can be found. 

```{r}
library(utils)
library(pracma)

aucs = c(glm_ROC$auc, bagging_ROC$auc, roc_curve$auc)
aucnames = c("Logistic regression", "Bagging", "Neural network")

print("The AUCs: ")
for (i in c(1,2,3)){
  print(paste(aucnames[i], sep = ": ", round(aucs[i], 3)))
}
```
All methods have a relatively high AUC, which shows that they all work relatively well. We see that bagging has a slightly lower AUC values than logistic regression and the neural network, whihc indicates that the two latter are preferrable. 
It is then natural to conclude that logisic regression is the best option for our data, since it scores well on both misclassification rate and AUC. 
