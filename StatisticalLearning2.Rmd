...
**Q5:** 
We now look at the main differences between using AIC and using Cross-validation for model selection. Cross-validation is in general more computationally expensive than model selection with AIC, however, this is rearly considered a problem with today's computers. Cross-validation also measures the test error directly, instead of adjusting the training error for model size, as the AIC does. Finally, cross-validation makes fewer assumptions about the underlying model. Cross-validation does not, unlike AIC, directly punish models with many covariates. If two models with different complexity produces the same test error, cross-validation will do nothing to choose the less complex model. 
...
**Q11:** When using a greedy approach one always takes the locally best choice, not thinking about what happens next or in the big picture. When using a greedy approach constructing a regression tree, one will always make the best splits, meaning one will choose the split that reduces the RSS the most. 
...
KAN NOEN ANDRE ENN JOHANNE SE OVER DENNE?? spesielt om jeg har svart på alle spørsmålene. 
...
**Q16:** In random forest one will have to choose the number of trees, B. A large B will not lead to overfitting, and therefore B is chosen sufficiently large, so that the error rate has settled. 
...
**Q18:** When using random forest, one usually choose $m \approx \sqrt{p}$, which in this case means that $ m \approx \sqrt{9} = 3$. Also, the number of trees in the forest, B, needs to be chosen. A big $B$ will not lead to overfitting, so we need to choose it such that the error has stabilised. Common choices are $B=300$ and $B=500$, and here $B=300$ should be sufficient. 
...
We see that the regression tree has a substantially higher MSE than the rest, and that the random forest has the best MSE. This also indicated that the regression tree will provide us with the least information about the relation between the covariates and the price. This coincides with what we have previously observed, where the regression tree only takes two covariates into account in the prediction. 
...
library(e1071)
set.seed(2)
CV_radial = tune(svm, diabetes ~ ., data=ctrain, kernel="radial", ranges=list(gamma=c(10^-5, 10^-4,0.001,0.01,0.1,1,2,3,4,5), cost=c(0.001, 0.01, 0.1, 1, 5, 10, 20,40,80,100,120,140,160)))
#summary(CV_radial)
...
cv_nn = tune(nnet, diabetes ~ ., data=nn_train.df, ranges=list(size=1:40))
