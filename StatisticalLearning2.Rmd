...
```{r}
price_lasso=lm(logprice~logcarat+cut+clarity+color+depth+table+xx+yy+zz-1, data=dtrain)
#summary(price_lasso)
#tror kanskje heller du skal lage en matrise som du kan bruke lasso på. den må være ant.obs x 10(?) stor. 
# x = matrix(rnorm(n*p), nrow=n, ncol=p)?? fra yutube. n = ant samples, p=ant parameters. 
summary(dtrain)
x = matrix()
