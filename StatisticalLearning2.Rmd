...
log kanskje best siden boxcox er nærmere 1 for det. hvis boxcox er nærme 1 er den fornøyd.
vil heller ikke se noe mønster i residualplottet, og vi ser mindre mønster for logprice en for price. 
vil derfor velge logprice. 
```{r plott}
#plot(dtrain$price, dtrain$carat)
#plot(dtrain$logprice, dtrain$carat)
price.lm = lm(price ~ carat + color + clarity + cut, data=dtrain)
rresprice = rstudent(price.lm)
plot(price.lm$fitted.values, rresprice)
library(MASS)
boxcox(price.lm, plotit=TRUE)
#summary(price.lm)
logprice.lm = lm(logprice ~ logcarat + color + clarity + cut, data=dtrain)
rreslogprice = rstudent(logprice.lm)
plot(logprice.lm$fitted.values, rreslogprice)
library(MASS)
boxcox(logprice.lm, plotit=TRUE)
#summary(logprice.lm)


plot(logprice.lm$fitted.values, dtrain$carat)
plot(logprice.lm$fitted.values, dtrain$logcarat)
plot(logprice.lm$fitted.values, dtrain$color)
plot(logprice.lm$fitted.values, dtrain$clarity)
plot(logprice.lm$fitted.values, dtrain$cut)
