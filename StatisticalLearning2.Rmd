...
#Normalizing the data to make them usable for the built-in R-functions and resetting them to data-frames
...
cv_nn = tune(nnet, diabetes ~ ., data=nn_train.df, ranges=list(size=1:15))

nn_best_model=cv_nn$best.model
nn_best_model
nn_prob_test=predict(nn_best_model, newdata=nn_test.df) #jeg får ikke til , type="response"
...
plot.nnet(nn_best_model)


```
We see from the results that the size of the hidden layer is set to only one node and the misclassification rate is 19,8% for the fitted model. This means that a quite small model was enough to predict as well as earlier models. It is 
