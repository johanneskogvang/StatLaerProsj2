...
randFor.logprice=randomForest(~logcarat+cut+clarity+color+depth+table+xx+yy+zz, data=dtrain, mtry=3, importance=TRUE) #dette funker ikkke???
randFor.logprice$confusion
...
```
Making a new model based on the significant covariates:
First one with the covariates with significance 0.001, then another with covaritaes with significance 0.001 and the last with 0.1.
hvordan vet man hvem av de man skal velge???
```{r}
diab.glm2 = glm(diabetes ~ glu+ped, data=ctrain, family = "binomial")
#summary(diab.glm2)
#AIC = 298.52

diab.glm3=glm(diabetes ~ glu+ped+bmi, data=ctrain, family = "binomial")
#summary(diab.glm3)
#AIC = 281.4

diab.glm4=glm(diabetes ~ glu+ped+bmi+npreg+age, data=ctrain, family = "binomial")
summary(diab.glm4)
#her blir AIC lavere enn hvis man tar med alle variablene.
#AIC=267.89


```

Bruker diab.glm4 videre fordi den har lavest AIC. Vet ikke om det er riktig, velg en annen senere hvis det viser seg å være bedre. 
```{r}
diab.glm4_prob=predict(diab.glm4, newdata=ctest)
